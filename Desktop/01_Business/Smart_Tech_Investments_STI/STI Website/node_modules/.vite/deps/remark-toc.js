...
function search(root, expression, settings) {
  ...
  visit(root, "heading", function(node, position, parent) {
    ...
    if (position !== void 0 && opening && !endIndex && node.depth <= opening.depth) {
      endIndex = position;
    }
    if ((endIndex || !expression) && (!settings.minDepth || node.depth >= settings.minDepth) && (!settings.maxDepth || node.depth <= settings.maxDepth) && (!skip || !skip.test(value))) {
      map.push({ depth: node.depth, children: node.children, id: slug2 });
    }
  });
  return {
...
function toc(tree, options) {
  ...
  const heading = settings.heading ? toExpression(settings.heading) : void 0;
  const result = search(tree, heading, settings);
  return {
    index: heading ? result.index : void 0,
    endIndex: heading ? result.endIndex : void 0,
...
export {
  remarkToc as default
};
//# sourceMappingURL=remark-toc.js.map
