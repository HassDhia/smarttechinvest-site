...
export function ActivationAngleCTA({ categoryHint }: { categoryHint?: string | null }) {
  ...
  return (
    <div className="rounded-3xl border border-[hsl(var(--border))] bg-[hsl(var(--card))] p-6 sm:p-8 shadow-[0_15px_35px_rgba(0,0,0,0.35)]">
      <p className="text-xs uppercase tracking-[0.35em] text-[hsl(var(--foreground-tertiary))] mb-3">Activation angle</p>
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
        <a
          ...
          className="inline-flex items-center justify-center rounded-full bg-white px-7 py-3 text-xs font-semibold uppercase tracking-[0.35em] text-black shadow-[0_10px_30px_rgba(255,255,255,0.4)] transition hover:translate-y-px focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)] focus-visible:ring-offset-2 focus-visible:ring-offset-[hsl(var(--card))]"
        >
          Request a collaboration review
        </a>
        <p className="text-xs text-[hsl(var(--foreground-secondary))]">If it&apos;s a fit, we reply within 48 hours.</p>
      </div>
    </div>
  );
}
